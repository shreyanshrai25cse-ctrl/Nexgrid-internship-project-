...
export function AdjustmentModal({ open, onOpenChange }: { open: boolean; onOpenChange: (v: boolean) => void }) {
  const [isSubmitting, setIsSubmitting] = React.useState(false)
  const [error, setError] = React.useState<string | null>(null)
  const [success, setSuccess] = React.useState<string | null>(null)
  const [form, setForm] = React.useState({ productId: "", warehouseId: "", countedQuantity: "" })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/inventory/adjust", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ productId: form.productId, warehouseId: form.warehouseId, countedQuantity: parseInt(form.countedQuantity), userId: "user-1" }) })
      const data = await res.json()
      if (!data.success) { setError(data.error || "Failed"); return }
      const diff = data.data?.difference
      setSuccess(`Adjustment done! Difference: ${diff > 0 ? "+" : ""}${diff}`)
      mutate(); setForm({ productId: "", warehouseId: "", countedQuantity: "" })
        ...
        <form onSubmit={handleSubmit}>
          <div className="grid gap-4 py-4">
            ...
            <div className="grid gap-2">
              ...
              <Input type="number" min="0" placeholder="Physical count" className="bg-secondary" value={form.countedQuantity} onChange={(e) => setForm({ ...form, countedQuantity: e.target.value })} required />
            </div>
          </div>
          <DialogFooter>
            <Button type="button" variant="outline" onClick={() => onOpenChange(false)} disabled={isSubmitting}>Cancel</Button>
            <Button type="submit" disabled={isSubmitting}>{isSubmitting && <Spinner className="mr-2" />}Apply Adjustment</Button>
          </DialogFooter>
        </form>
      </DialogContent>
    </Dialog>
