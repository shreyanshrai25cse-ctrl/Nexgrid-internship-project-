...
export function ReceiptModal({ open, onOpenChange }: { open: boolean; onOpenChange: (v: boolean) => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/inventory/receipt", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ productId: form.productId, warehouseId: form.warehouseId, quantity: parseInt(form.quantity), userId: "user-1" }) })
      const data = await res.json()
      if (!data.success) { setError(data.error || "Failed"); return }
      setSuccess(`Receipt processed! Reference: ${data.data?.reference}`)
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[450px]">
        <DialogHeader><DialogTitle>📦 New Receipt — Incoming Stock</DialogTitle></DialogHeader>
        <form onSubmit={handleSubmit}>
          ...
          <DialogFooter>
            ...
            <Button type="submit" disabled={isSubmitting}>{isSubmitting && <Spinner className="mr-2" />}Process Receipt</Button>
          </DialogFooter>
        </form>
      </DialogContent>
    </Dialog>
